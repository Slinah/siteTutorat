...
<body>
    ...
    <div class="container">
        ...
        foreach (selectCoursMatiereNiveauBySecuIdPersonneRang($_GET['course'], $_SESSION['id_personne'], 1) as $c) {
            echo '<h3>Clore le cours - ' . $c['intitule'] . '</h3>';
            echo '<form action="updateCourseClosed.php?course=' . $c['id_cours'] . '" method="post">';
            ...
            echo '<br><button class="button" onclick="location.href = `updateCourseClosed.php?course=' . $c['id_cours'] . '`;"><span class="mif-checkmark"></span> Modifier le cours</button>';
        }
        ?>
        </form>
